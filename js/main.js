...
function shopData() {
    return {
        ...
        filterProducts() {
            ...
            this.filteredProducts = this.products.filter(p => p.name.toLowerCase().includes(lowerCaseSearch) || (p.sku && p.sku.toLowerCase().includes(lowerCaseSearch)));
        },
        ...
        addToCartFromDetail() {
            // Logic to add to cart from the detail modal
            const productToAdd = { 
                ...this.selectedProduct, 
                quantity: 1,
                selectedOptions: this.selectedOptions,
                printing_notes: '' // Reset printing notes
            };
        ...
        confirmPrintingAndAddToCart() {
            ...
            let printingNotes = '';
            if (name || number) {
                printingNotes = `Tên: ${name || 'N/A'}, Số: ${number || 'N/A'}`;
            }
